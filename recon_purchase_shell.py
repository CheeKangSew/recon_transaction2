...
def load_and_prepare_data(file1, file2):
    ...
    df1['Transaction Date'] = pd.to_datetime(df1['Delivery Date'], format='%d/%m/%Y')
    df1['Transaction Time'] = pd.to_datetime(df1['Time'], format='%H:%M:%S').dt.time
    ...
    df1['Net Amount in Customer currency'] = pd.to_numeric(df1['Net Amount in Customer currency'], errors='coerce')
    ...
    df1['Vehicle License Number'] = df1['Vehicle License Number'].str.replace(r'\s+', '', regex=True)
...
def add_matched_column(df1, matched_transactions):
    ...
    df1['Matched'] = df1.apply(
        lambda row: any(
            ...
            (matched_transactions['Amount1'] == row['Net Amount in Customer currency']) &
            (matched_transactions['VehicleNumber1'] == row['Vehicle License Number'])
        ), axis=1
    )
...
def main():
    ...
    # Upload files
    file1 = st.file_uploader("Upload the fleetcard CSV file from Shell", type="csv")
    ...
    if file1 and file2:
        ...
        for index, row in mismatched_transactions.iterrows():
            df1_with_matched.loc[(df1_with_matched['TransactionDateTime'] == row['TransactionDateTime']) &
                                 (df1_with_matched['Net Amount in Customer currency'] == row['Amount1']) &
                                 (df1_with_matched['Vehicle License Number'] == row['VehicleNumber1']), 'MismatchReason'] = row['MismatchReason']
        
        # Display the first file with matched column and mismatch reasons
        st.subheader("Shell File with Matched Column and Mismatch Reasons")
        ...
        st.download_button(
            label="Download Shell File with Matched Column and Mismatch Reasons",
